...
import ejercicios.NVPNaive;
import ejercicios.SVPNaive;
...
public class TestLattices {
    ...
    public static void genDataNVP() {
        ...
        // Función que mide el tiempo de ejecución
        Function<Integer, Long> f1 = dim -> {
            List<List<Integer>> base = NVPNaive.generateRandomBase(dim); // Base aleatoria
            List<List<Integer>> latticePoints = NVPNaive.generateLatticePoints(base, range);
            List<Integer> randomPoint = NVPNaive.generateRandomPoint(dim, dim);
            long startTime = System.nanoTime();
            NVPNaive.findNearestVector(latticePoints, randomPoint);
            long endTime = System.nanoTime();
